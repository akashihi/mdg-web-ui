...
export default class TransactionShortWidget extends TransactionBase {
    render() {
        ...
        return <Grid>
            <Row>
                <Col xs={3} sm={2} md={1} lg={3}>{timestampToFormattedDate(transaction.get('timestamp'))}</Col>
                <Col xs={6} sm={3} md={3} lg={3}>{transaction.get('comment')}</Col>
                <Col xs={3} sm={1} md={1} lg={2}>
                    <div style={{color: totals.color}}>{totals.total}</div>
                </Col>
                <Col xs={7} sm={3} md={2}
                     lg={2}>{::this.renderTransactionAccountList(transaction.get('operations'), props.accounts)}</Col>
